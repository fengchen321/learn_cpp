...
private:
    _Tp *_M_p;
    _Deleter _M_deleter;  // [[no_unique_address]]  c++20
...
public:
    ...
    template <class _Up, class _UDeleter> requires (std::convertible_to<_Up *, _Tp *>)
#else
    template <class _Up, class _UDeleter, class = std::enable_if_t<std::is_convertible_v<_Up *, _Tp *>>>
#endif
...
template <class _Tp, class ..._Args, std::enable_if_t<!is_unbounded_array_v<_Tp>, int> = 0>
UniquePtr<_Tp> makeUnique(_Args &&...__args) {
    return UniquePtr<_Tp>(new _Tp(std::forward<_Args>(__args)...));
}

template <class _Tp, std::enable_if_t<!is_unbounded_array_v<_Tp>, int> = 0>
